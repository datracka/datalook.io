...
function nb_fix_serialization($string) {
	//return preg_replace('!s:(\d+):"(.*?)";!e', "'s:'.strlen('$2').':\"$2\";'", $string); // global chars recalculation
	return str_replace('&amp;', '&', $string);	
}


// get public custom post types and taxonomies
function nb_get_cpt_with_tax() {
	$cpt = get_post_types(array('public' => true, '_builtin' => false), 'objects');
	$usable = array(); 
...
// given CPT+taxonomy slugs, returns human names
function nb_cptTax_to_cpt_name($cpt_tax) {
	$arr = explode('|||', $cpt_tax);
	
	$cpt = get_post_type_object($arr[0]);
	$cpt = $cpt->labels->name;	
	
	$tax = get_taxonomy($arr[1]);
	$tax = $tax->labels->name;
...
function nb_create_custom_theme() {	
	...
	$file = NB_DIR . '/js/sb/themes/wpdt.css';
...
function nb_news_sources($src = '') {
	...
	// add custom post type sources
	$cpt = get_option('nb_integrate_cpt'); 
	if(!empty($cpt)) {
		foreach($cpt as $pt) {
			$sources[$pt] = nb_cptTax_to_cpt_name($pt);	
		}
	}
...
function nb_src_form_block($data) {
	...
	else if(strpos($data['src_type'], '|||') !== false) { // custom post type + taxonomy
...
function nb_inline_code($src, $limit, $man_exp_img) {
	...
	/*** query ***/
	if(strpos($src['src_type'], '|||') !== false) {
		$cpt_arr = explode('|||', $src['src_type']);
		$pt = $cpt_arr[0];	
	}
	else {
		switch($src['src_type']) {
	...
	$args = array(
		...
		'post_type' 	=> $pt,
		'offset'		=> (isset($src['news_offset'])) ? (int)$src['news_offset'] : 0  
	);
	...
	if(!empty($src['src_val'])) {
		...
		else { // custom post type + taxonomy
			$args['tax_query'] = array(
				array(
					'taxonomy' => $cpt_arr[1],
					'field' => 'id',
	...
	// strip/remove tags
	$str_rm_tags = '';
	if(isset($src['strip_tags']) && !empty($src['strip_tags'])) {$str_rm_tags .= 'strip_tags="'. trim($src['strip_tags']) .'"';}
	if(isset($src['remove_tags']) && !empty($src['remove_tags'])) {$str_rm_tags .= ' remove_tags="'. trim($src['remove_tags']) .'"';}
	
	$code = '<div class="nb_news_wrap" style="display: none;" link_target="'.$src['link_target'].'" '.$img_chk_attr.' '.$exp_img_attr.' '.$str_rm_tags.'>';
	...
	foreach($query as $post) {
		...
		if(has_post_thumbnail($post->ID)) {
			...
			$img = '<img src="'.$img_src[0].'" class="lcnb_main_img" />';
		} 
		else {$img = '';}
		...
		// author
		if(!empty($src['author'])) {
			$author = '<span class="lcnb_author">'. str_replace(array('&apos;', '&quot;', '&lt;', '&gt;'), array('\'', '"', '<', '>'), $src['author']) .'</span> ';
		}
		else {$author = '';}
		
		// excerpt
		if(!empty($post->post_excerpt)) {
			$excerpt = '<blockquote>'. $author . nl2br(trim($post->post_excerpt)) .'</blockquote>';	
		}
		else {$excerpt = '';}
		...
		<article datetime="'.get_post_time('c', true, $post->ID).'">
            ...
            <section>
				'.$author . do_shortcode(wpautop(trim($post->post_content))).'
			</section>
			'.$excerpt.'
			'.$img.'
			'.$media.'
...
function nb_types_opt($type) {
	$opts = array(
		...
		'src_hide_elements' => array(
			...
			'opts' => array('title' => __('title', 'nb_ml'), 'link'=> __('link', 'nb_ml')),
			'multiple' => true,
			'optional' => true
		),
		'link_target' => array(
		...
		'self_s_link' => array(
			'type' => 'bool',
			'label' => __("Set page's URL as sharing link?", 'nb_ml'),
		),
		'strip_tags' => array(
			'type' => 'text',
		...
		'layout' => array(
			...
			'opts' => array(
				'horizontal' => __("horizontal", 'nb_ml'), 
				'vertical' => __('vertical', 'nb_ml')
			),
		),
		'boxed_news' => array(
			'type' => 'bool',
		...
		'horiz_img_mode' => array(
			'type' => 'bool',
			'label' => __("Horizontal image mode?", 'nb_ml'),
		),
		...
		'hide_elements' => array(
			...
			'opts' => array(
				'date' => __("date", 'nb_ml'), 
				'title' => __('title', 'nb_ml'), 
				'image' => __('main image', 'nb_ml'), 
				'link' => __('link', 'nb_ml'), 
				'socials' => __('social share', 'nb_ml'),
				'btn_bar' => __('buttons bar', 'nb_ml')
			),
			'multiple' => true,
			'optional' => true
		),
		...
		'pause_on_hover' => array(
			'type' => 'bool',
			'label' => __('Pause slideshow on hover?', 'nb_ml')
		),
		'slide_all' => array(
			'type' => 'bool',
			'label' => __('Slide all visible elements per time? <small>(only in slideshow)</small>', 'nb_ml')
		),
		'autop_after_exp' => array(
			'type' => 'bool',
			'label' => __('Resume autoplay after expanded news closing?', 'nb_ml')
		),
	);
