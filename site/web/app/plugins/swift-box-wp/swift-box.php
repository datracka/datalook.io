...
function nb_global_scripts() {
	...
	if (is_admin()) {  
		...
		// lcweb switch
		wp_enqueue_style( 'lc-switch', NB_URL.'/js/lc-switch/lc_switch.css', 999);
	...
	if (!is_admin()) {
		...
		if(get_option('nb_js_head') != '1') {
			wp_enqueue_script('nb-core', NB_URL.'/js/sb/swift-box.min.js', 100, '1.124-1.324', true);	
		} else { 
			wp_enqueue_script('nb-core', NB_URL.'/js/sb/swift-box.min.js', 99, '1.124-1.324');
		}
		...
		// core CSS
		wp_enqueue_style('nb-core',  NB_URL.'/js/sb/swift-box-layout.min.css', 900);

		// frontend custom css inline - custom font sizes
...
function nb_inline_css(){
	...
	// if custom font sizes are used
	$cust_font_code = '';
	if(get_option('nb_is_title_size', 14) != 14) {$cust_font_code .= '.lcnb_linked_title, .lcnb_title {font-size: '.get_option('nb_is_title_size', 14).'px;} ';}
	if(get_option('nb_is_txt_size', 12) != 12) {$cust_font_code .= '.lcnb_txt {font-size: '.get_option('nb_is_txt_size', 12).'px;} ';}
	if(get_option('nb_es_title_size', 21) != 21) {$cust_font_code .= '.lcnb_exp_block .lcnb_title, .lcnb_exp_block .lcnb_linked_title {font-size: '.get_option('nb_es_title_size', 21).'px;} ';}
	if(get_option('nb_es_txt_size', 13) != 13) {$cust_font_code .= '.lcnb_exp_txt {font-size: '.get_option('nb_es_txt_size', 13).'px;} ';}

	// if locked server - use custom theme inline
	if((get_option('nb_inline_css') || get_option('nb_force_inline_css')) && get_option('nb_custom_style')) {
	...
	// custom CSS
	$custom_css = get_option('nb_custom_css') . $cust_font_code;
...
function nb_footer_js(){
	...
	nb_script_basepath = <?php echo '"'.NB_URL.'/js/sb/"' ?>;
...
$lcun = new lc_update_notifier(__FILE__, 'http://www.lcweb.it/envato_update/nb-wp.php');
////////////
...
function nb_remove_wp_helper() {
	...
	$hooked = array('swift-news_page_nb_settings', 'toplevel_page_nb_menu');
