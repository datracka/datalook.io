...
<div class="wrap lcwp_form">  
    ...
    <?php echo '<h2 class="lcwp_page_title" style="border: none;">Swift Box - ' . __( 'Manage Boxes', 'nb_ml') . "</h2>"; ?>  
          ...
          <form class="form-wrap">	
           ...
           <div class="postbox lcwp_sidebox_meta">
				...
				<div class="inside">
                    ...
                    <input type="button" name="add_box" id="add_box" value="<?php _e('Add', 'nb_ml') ?>" class="button-primary" style="margin-left: 5px;" />
                </div>
            </div>
...
jQuery(document).ready(function($) {
	...
	jQuery('body').delegate('#save_nb_box input', 'click', function() {
		...
		// sanitize & in source data
		src_data = JSON.stringify(src_data);
		src_data = encodeURIComponent(src_data);

		// save
		jQuery('#save_nb_box div').html('<div style="height: 30px;" class="lcwp_loading"></div>');
		var data = 'action=nb_save_box&box_id='+ sel_box +'&src_data='+ src_data +'&'+ jQuery('#nb_box_settings').find('input, select, textarea').serialize();
		...
		jQuery.post(ajaxurl, data, function(response) {
			...
			if(resp == 'success') {
				...
				jQuery('#ajax_mess').empty().append(resp);
			}
		});
	});
