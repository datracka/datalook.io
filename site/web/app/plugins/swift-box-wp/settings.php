...
<div class="wrap lcwp_form">  
    ...
    <?php echo '<h2 class="lcwp_page_title" style="border: none;">Swift Box ' . __('Settings', 'nb_ml') . "</h2>"; ?>  
	...
	if(isset($_POST['lcwp_admin_submit'])) { 
		...
		$indexes[] = array('index'=>'nb_min_horiz_w', 'label'=>__("Minimum horizontal Swift Box width", 'nb_ml'), 'type'=>'int');
		...
		$indexes[] = array('index'=>'nb_integrate_cpt', 'label'=>'CPT integration');	
		
		$indexes[] = array('index'=>'nb_is_title_size', 'label'=>__("Initial mode - title font size", 'nb_ml'), 'type'=>'int');
		$indexes[] = array('index'=>'nb_is_txt_size', 'label'=>__("Initial mode - text font size", 'nb_ml'), 'type'=>'int');
		$indexes[] = array('index'=>'nb_es_title_size', 'label'=>__("Expanded mode - title font size", 'nb_ml'), 'type'=>'int');
		$indexes[] = array('index'=>'nb_es_txt_size', 'label'=>__("Expanded mode - text font size", 'nb_ml'), 'type'=>'int');
		...
		$fdata['nb_integrate_cpt'] = get_option('nb_integrate_cpt'); 
		
		$fdata['nb_is_title_size'] = get_option('nb_is_title_size', 14);
		$fdata['nb_is_txt_size'] = get_option('nb_is_txt_size', 12);
		$fdata['nb_es_title_size'] = get_option('nb_es_title_size', 21);
		$fdata['nb_es_txt_size'] = get_option('nb_es_txt_size', 13);
    ...
    <div id="main_opt"> 
        ...
        <table class="widefat lcwp_table">
          ...
          <tr>
            <td class="lcwp_label_td"><?php _e("Minimum horizontal Swift Box width", 'nb_ml'); ?></td>
            <td class="lcwp_field_td">
        ...
        <?php 
		$cpt = nb_get_cpt_with_tax();
		if(!empty($cpt)) :
		?>
        <h3><?php _e("Custom post types integration", 'nb_ml'); ?></h3>
        <table class="widefat lcwp_table">
		  ...
		  foreach($cpt as $slug => $data) {
			echo '
			<tr>
				<td class="lcwp_label_td">'. $data['name'] .'</td>
				<td class="lcwp_field_td">
					<select name="nb_integrate_cpt[]" class="lcweb-chosen" data-placeholder="'. __("Select a taxonomy for this custom post type", 'nb_ml') .' .." multiple="multiple" style="width: 90%; max-width: 500px;">';
						
						foreach($data['tax'] as $tax_slug => $tax_name) {
							$val = $slug.'|||'.$tax_slug;
							$sel = (is_array($fdata['nb_integrate_cpt']) && in_array($val, $fdata['nb_integrate_cpt'])) ? 'selected="selected"' : '';
                      		echo '<option value="'.$val.'" '.$sel.'>'.$tax_name.'</option>';	
						}
    ...
    <div id="styling"> 
    	<h3><?php _e("Font size control", 'nb_ml'); ?></h3>
        <table class="widefat lcwp_table">
          <tr>
            <td class="lcwp_label_td"><?php _e("Initial mode - title", 'nb_ml'); ?></td>
            <td class="lcwp_field_td">
            	<div class="lcwp_slider" step="1" max="20" min="10"></div>
                <input type="text" value="<?php echo $fdata['nb_is_title_size']; ?>" name="nb_is_title_size" class="lcwp_slider_input" />
                <span>px</span>
            </td>
            <td><span class="info"><?php _e("Custom title font size on initial mode (default: 14px)", 'nb_ml') ?></span></td>
          </tr>
          <tr>
            <td class="lcwp_label_td"><?php _e("Initial mode - text", 'nb_ml'); ?></td>
            <td class="lcwp_field_td">
            	<div class="lcwp_slider" step="1" max="17" min="10"></div>
                <input type="text" value="<?php echo $fdata['nb_is_txt_size']; ?>" name="nb_is_txt_size" class="lcwp_slider_input" />
                <span>px</span>
            </td>
            <td><span class="info"><?php _e("Custom text font size on initial mode (default: 12px)", 'nb_ml') ?></span></td>
          </tr>
          <tr>
            <td class="lcwp_label_td"><?php _e("Expanded mode - title", 'nb_ml'); ?></td>
            <td class="lcwp_field_td">
            	<div class="lcwp_slider" step="1" max="30" min="15"></div>
                <input type="text" value="<?php echo $fdata['nb_es_title_size']; ?>" name="nb_es_title_size" class="lcwp_slider_input" />
                <span>px</span>
            </td>
            <td><span class="info"><?php _e("Custom title font size on expanded mode (default: 21px)", 'nb_ml') ?></span></td>
          </tr>
          <tr>
            <td class="lcwp_label_td"><?php _e("Expanded mode - text", 'nb_ml'); ?></td>
            <td class="lcwp_field_td">
            	<div class="lcwp_slider" step="1" max="18" min="10"></div>
                <input type="text" value="<?php echo $fdata['nb_es_txt_size']; ?>" name="nb_es_txt_size" class="lcwp_slider_input" />
                <span>px</span>
            </td>
            <td><span class="info"><?php _e("Custom text font size on expanded mode (default: 13px)", 'nb_ml') ?></span></td>
          </tr>
        </table>
...
<script src="<?php echo NB_URL; ?>/js/lc-switch/lc_switch.min.js" type="text/javascript"></script>
...
jQuery(document).ready(function($) {
	...
	jQuery('body').delegate('#nb_show_date_helper', "click", function (e) {
		e.preventDefault();
		tb_show('Swift Box date helper', '#TB_inline?height=600&width=640&inlineId=nb_date_helper');
		setTimeout(function() {
